Public Class frm_occupation
    ...
    Private Sub Selection()
        ...
        Try
            obj.BindDataGridView("SELECT OCCUPATION_ID,ROW_NUMBER() OVER(ORDER BY OCCUPATION_ID asc) AS 'ROW_NUM', OCCUPATION_KH, OCCUPATION_EN FROM dbo.TBL_OCCUPATION", datagrid)
            datagrid.Columns(0).Visible = False
            datagrid.Columns(1).HeaderText = "ល.រ"
            datagrid.Columns(2).HeaderText = "មុខរបរ (ខ្មែរ)"
            datagrid.Columns(3).HeaderText = "មុខរបរ (ឡាតាំង)"
        ...
        Try
            If datagrid.SelectedRows.Count > 0 Then
                ...
                If USER_CLICK_OK = True Then
                    obj.Delete("DELETE FROM  dbo.TBL_OCCUPATION WHERE  OCCUPATION_ID = " & datagrid.SelectedRows(0).Cells(0).Value & "")
    ...
    Private Sub frm_occupation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Call Selection()
        Catch ex As Exception
