Public Class frm_professional_level
    ...
    Private Sub Selection()
        ...
        Try
            obj.BindDataGridView("SELECT PROFESSIONAL_ID,ROW_NUMBER() OVER(ORDER BY PROFESSIONAL_ID asc) AS 'ROW_NUM',PROFESSIONAL_KH,PROFESSIONAL_EN FROM dbo.TBL_PROFESSIONAL_LEVEL", datagrid)

            datagrid.Columns(0).Visible = False
            datagrid.Columns(1).HeaderText = "ល.រ"
            datagrid.Columns(2).HeaderText = "កម្រិតវិជ្ជាជីវៈ (KH)"
            datagrid.Columns(3).HeaderText = "កម្រិតវិជ្ជាជីវៈ (EN) "
        ...
        Try
            If datagrid.SelectedRows.Count > 0 Then
                ...
                If USER_CLICK_OK = True Then
                    obj.Delete("DELETE FROM  dbo.TBL_PROFESSIONAL_LEVEL WHERE  PROFESSIONAL_ID = " & datagrid.SelectedRows(0).Cells(0).Value & "")
