Public Class frm_subject
    ...
    '        txtTitleKh.Text = DataGrid.SelectedRows(0).Cells(2).Value
    '        txtTitleEn.Text = DataGrid.SelectedRows(0).Cells(3).Value
    '        txtMin12.Text = DataGrid.SelectedRows(0).Cells(4).Value
    '        txtMax12.Text = DataGrid.SelectedRows(0).Cells(5).Value
    '        txtMulti12.Text = DataGrid.SelectedRows(0).Cells(6).Value
    '        txtMax.Text = DataGrid.SelectedRows(0).Cells(7).Value()
    '        txtMin.Text = DataGrid.SelectedRows(0).Cells(8).Value()
    '        txtMulti.Text = DataGrid.SelectedRows(0).Cells(9).Value()
    ...
    '        Try
    '            If txtTitleKh.Text = "" Or txtMin12.Text = "" Or txtMax12.Text = "" Or txtMax.Text = "" Or txtMin.Text = "" Or txtMulti.Text = "" Then
    '                obj.ShowMsg("???????????????????????", msg_fail, "Error.wav")
    '                txtTitleKh.Focus()
    '                Exit Sub
    '            End If

    '            obj.Insert_SQL("INSERT INTO dbo.TBL_SUBJECT(" & vbCrLf &
    '"SUBJECT_KH,SUBJECT_EN,GRADE_12_MIN_SCORE,GRADE_12_MAX_SCORE,GRADE_12_MULTIPLE," & vbCrLf &
    '"MIN_SCORE,MAX_SCORE,MULTIPLE" & vbCrLf &
    '")VALUES" & vbCrLf &
    '"(N'" & txtTitleKh.Text & "',N'" & txtTitleEn.Text & "'," & txtMin12.Text & "," & txtMax12.Text & "," & txtMulti12.Text & "," & txtMin.Text & "," & txtMax.Text & "," & txtMulti.Text & ")")
    '            Call Selection()
    '        Catch ex As Exception
    ...
    '            Exit Sub
    '        End If
    '        obj.Update_SQL("UPDATE dbo.TBL_SUBJECT SET SUBJECT_KH = N'" & txtTitleKh.Text & "',SUBJECT_EN=N'" & txtTitleEn.Text & "',GRADE_12_MIN_SCORE=" & txtMin12.Text & " ,GRADE_12_MAX_SCORE=" & txtMax12.Text & ",GRADE_12_MULTIPLE=" & txtMulti12.Text & ",MIN_SCORE=" & txtMin.Text & ",MAX_SCORE=" & txtMax.Text & ",MULTIPLE= " & txtMulti.Text & "  WHERE SUBJECT_ID = " & datagrid.SelectedRows(0).Cells(0).Value & " ")
    '        Call Selection()
    '    Catch ex As Exception
    '        error_reason = ex.Message
    '        obj.ShowMsg(ex.Message, frm_success, "")
    ...
    '                obj.Delete_SQL("DELETE FROM  dbo.TBL_SUBJECT WHERE  SUBJECT_ID = " & datagrid.SelectedRows(0).Cells(0).Value & "")
    ...
    '        End If
    '    Catch ex As Exception
    '        obj.ShowMsg(ex.Message, msg_fail, "Error.wav")
    '    End Try
    'End Sub
